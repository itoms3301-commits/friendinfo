...
- 「本」「イベント（旧・サークル）」はチェックボックスによる複数選択式です。金父・CFQはそれぞれ独立に「提案中→購入→読書中→読了」の進捗段階をプルダウンで記録できます（チェックすると自動で「提案中」が選ばれ、チェックを外すと段階もクリアされます）。データは新項目（`bookKinfu`/`bookCfq`/`bookOther`、`circleBistro`/`circleChange`/`circleOther`）に保存し、旧・単一選択形式の列（本（旧・互換用）／イベント（旧・互換用））は削除せず残しています。新項目が空の友達は、保存済みの旧データから一度きりの読み取り専用フォールバック表示を行います（実際にフォームを保存するまでは旧データへ書き戻しません）
  - 「現在の状態」はチップ選択からプルダウン（`<select>`）に変更しました
